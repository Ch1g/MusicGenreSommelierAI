...
| `frontend` | React/TypeScript SPA; build context [`frontend/`](../frontend/). **Dev** (`docker-compose.yml`): Vite dev server on port **8080**; bind-mount **`./frontend`** → **`/src`**; `node_modules` isolated via anonymous volume. **Prod** (`docker-compose.remote.yml`): multi-stage build via [`frontend/Dockerfile.prod`](../frontend/Dockerfile.prod) (`npm run build` → nginx:1.27-alpine serving `dist/`); [`frontend/nginx.conf`](../frontend/nginx.conf) mounted at `/etc/nginx/conf.d/default.conf` (listens on **8080**, SPA fallback via `try_files`). |
